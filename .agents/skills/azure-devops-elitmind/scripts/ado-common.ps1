...
function Get-AdoDefaultTimeoutSeconds {
    $configured = $env:ELITMIND_ADO_TIMEOUT_SECONDS
    ...
    try {
        if (-not (Wait-Job -Job $job -Timeout $TimeoutSeconds)) {
            ...
            throw ("Azure CLI timed out after $TimeoutSeconds seconds: az $($Arguments -join ' '). " +
                ...
                "Raise ELITMIND_ADO_TIMEOUT_SECONDS only when the operation is genuinely slow.")
        }
...
function Resolve-AdoOrg {
    ...
    if (-not [string]::IsNullOrWhiteSpace($env:ELITMIND_ADO_ORG)) {
        return $env:ELITMIND_ADO_ORG.TrimEnd("/")
    }
    ...
    return "https://dev.azure.com/Elitmindvs"
}
...
function Resolve-AdoProject {
    ...
    if (-not [string]::IsNullOrWhiteSpace($env:ELITMIND_ADO_PROJECT)) {
        return $env:ELITMIND_ADO_PROJECT
    }
    ...
    throw "Azure DevOps project could not be resolved. Pass -Project, set ELITMIND_ADO_PROJECT, or run from an Azure Repos checkout whose remote URL contains the project."
}
...
function Resolve-AdoRepository {
    ...
    if (-not [string]::IsNullOrWhiteSpace($env:ELITMIND_ADO_REPOSITORY)) {
        return $env:ELITMIND_ADO_REPOSITORY
    }
    ...
    throw "Azure DevOps repository could not be resolved. Pass -Repository, set ELITMIND_ADO_REPOSITORY, or run from an Azure Repos checkout whose remote URL contains the repository."
}
